...
EOF
# Xray работает под непривилегированным пользователем, поэтому конфиг
# с приватным ключом отдаём ему, а не оставляем читаемым всем.
chown nobody:nogroup "$XRAY_CONFIG" 2>/dev/null || chown nobody:nobody "$XRAY_CONFIG"
